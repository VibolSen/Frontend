...
export default function JobPostingsPage() {
  ...
  const fetchJobPostings = async () => {
    ...
    try {
      ...
      setStatusMessage({ type: "error", message: err.response?.data?.message || err.message });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await apiClient.delete(`/hr/job-postings/${jobToDelete}`);
              ...
              {jobPostings.map((job) => (
                <tr key={job.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {job.applicationDeadline ? format(new Date(job.applicationDeadline), "PPP") : 'No Deadline'}
                  </td>
