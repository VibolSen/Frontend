...
export default async function ExamGradingPage({ params }) {
  const awaitedParams = await params;
  const { examId, submissionId } = awaitedParams;
    ...
    return (
      <div className="text-center p-8">
        ...
        <div className="mt-6 flex justify-center">
          <BackButton href={`/teacher/exam/${examId}`} label="Back to Exam Roster" />
        </div>
      </div>
    );
  }

  return <ExamGradingView initialSubmission={submission} backLink={`/teacher/exam/${examId}`} />;
}
