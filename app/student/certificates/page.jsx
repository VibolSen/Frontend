...
const MyCertificatesPage = () => {
  ...
  // Use the backend URL for direct downloads
  ...
  useEffect(() => {
    const fetchCertificates = async () => {
      if (!user) return;
      try {
        const data = await apiClient.get(`/certificates?userId=${user.id}`);
            ...
            {filteredCertificates.map((certificate, index) => (
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-2.5">
                  ...
                  <Link 
                    href={`${BACKEND_URL}/api/certificates/${certificate.id}/download`}
                    target="_blank"
