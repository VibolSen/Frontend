...
const NavLink = ({ icon, label, href, isCollapsed, isActive }) => (
  <li>
    ...
    >
      <div
        className={`flex items-center justify-center shrink-0 transition-transform duration-300 ${isActive ? "scale-110" : "group-hover:scale-110"
          }`}
      >
...
export default function FinanceSidebar({ isOpen, setIsOpen }) {
  ...
  return (
    <>
      <div
        className={`fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-30 transition-opacity md:hidden ${isOpen ? "opacity-100" : "opacity-0 pointer-events-none"
          }`}
      ...
      >
        ...
        {/* Header */}
        <div
          className={`flex items-center px-6 border-b border-slate-200 h-24 transition-all duration-300 relative z-10 ${isCollapsed ? "justify-center" : "justify-between"
            }`}
        >
          {!isCollapsed && (
            <motion.div
            ...
            >
              <div className="h-10 w-10 bg-blue-600 rounded-xl flex items-center justify-center shadow-lg shadow-blue-500/20 -rotate-3 transition-transform hover:rotate-0">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[8px] text-slate-500 font-black uppercase tracking-[0.3em] mt-1">Financial Office</span>
              </div>
            </motion.div>
          )}
        ...
        <div className="p-4 border-t border-slate-200 relative z-10 bg-white/50 backdrop-blur-md">
          ...
          >
            <div className="relative shrink-0">
              <img
                ...
                className="w-9 h-9 rounded-xl object-cover ring-2 ring-slate-100 group-hover:ring-blue-200 transition-all"
                alt="Profile"
              />
            ...
            {!isCollapsed && (
              <div className="flex-1 min-w-0">
                <p className="text-[11px] font-black text-slate-800 truncate group-hover:text-blue-700 transition-colors uppercase italic">{user?.firstName} {user?.lastName}</p>
          ...
          {!isCollapsed && (
            <button
              ...
              className="mt-3 w-full flex items-center justify-center gap-2 p-2.5 rounded-xl border border-slate-100 bg-white/50 text-slate-400 hover:text-rose-500 hover:bg-rose-50 hover:border-rose-100 transition-all font-black text-[10px] uppercase tracking-widest shadow-sm"
            >
              <LogOut size={14} />
              Sign Out
            </button>
