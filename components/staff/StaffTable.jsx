...
import { Edit, Eye, Trash2, Search, Users, Filter } from "lucide-react";
    ...
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden transition-all">
      ...
      <div className="p-4 border-b border-slate-100 bg-slate-50/30 space-y-3">
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1 md:pb-0 scrollbar-hide">
          ...
          <div className="px-3 py-1.5 bg-blue-50 text-blue-700 text-[10px] font-black uppercase tracking-tight rounded-lg border border-blue-100 shrink-0">
                ...
                >
                  <td className="px-5 py-3 whitespace-nowrap">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-lg bg-blue-100 text-blue-700 flex items-center justify-center font-black text-[10px] shrink-0 border border-blue-200 shadow-sm shadow-blue-100">
                        {staff.firstName.charAt(0)}{staff.lastName.charAt(0)}
                      </div>
                      <div className="flex flex-col">
                        <span className="text-[13px] font-black text-slate-800 tracking-tight">{staff.firstName} {staff.lastName}</span>
                        <span className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">Verified Personnel</span>
                      </div>
                    </div>
                  </td>
                  <td className="px-5 py-3 whitespace-nowrap hidden md:table-cell">
                    <span className="text-[13px] font-semibold text-slate-600">{staff.email}</span>
                  </td>
                  <td className="px-5 py-3 whitespace-nowrap">
                    <span className={`px-2 py-0.5 text-[9px] font-black rounded-md border uppercase tracking-wide ${roleColors[staff.role] || "bg-slate-50 text-slate-700 border-slate-100"}`}>
                      {staff.role}
                    </span>
                  </td>
