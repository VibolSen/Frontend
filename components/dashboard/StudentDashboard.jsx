...
const StudentDashboard = ({ loggedInUser }) => {
  ...
  useEffect(() => {
    if (loggedInUser) {
      const fetchStudentDashboardData = async () => {
        try {
          // The backend /dashboards/student returns the data structure we need
          // But the frontend expects 'myProfile' which we can get from UserContext or /auth/me
          // For now, let's assume the backend dashboard returns what's needed or we merge them
          const data = await apiClient.get(
            `/dashboards/student?studentId=${loggedInUser.id}`
          );
          if (data) {
            // Merging with loggedInUser to satisfy 'myProfile' requirement in the component
  ...
  const { myProfile, pendingAssignmentsCount = 0, pendingExamsCount = 0 } = dashboardData;
  // Use enrollments from dashboardData if available
