...
export default function BulkCertificateModal({ isOpen, onClose, onCertificatesIssued, showMessage }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      showMessage(`Successfully issued ${result.count || selectedStudentIds.size} certificates!`, "success");
      onCertificatesIssued();
      onClose();
    } catch (error) {
      showMessage(error.response?.data?.error || error.message || "Failed to issue certificates", "error");
    } finally {
