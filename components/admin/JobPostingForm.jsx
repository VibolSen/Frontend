...
export default function JobPostingForm({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...formData,
        requirements: typeof formData.requirements === 'string' ? formData.requirements.split(",").map((req) => req.trim()).filter(Boolean) : formData.requirements,
        responsibilities: typeof formData.responsibilities === 'string' ? formData.responsibilities.split(",").map((res) => res.trim()).filter(Boolean) : formData.responsibilities,
      };

      if (initialData) {
        await apiClient.put(`/career/job-postings/${initialData.id}`, payload);
      } else {
        await apiClient.post("/career/job-postings", payload);
      }
