...
export default function AssignmentGradingView({ initialSubmission, backLink }) {
  ...
  const handleSaveGrade = async (e) => {
    ...
    setIsSaving(true);
    try {
      const updated = await apiClient.put(`/teacher/submissions/${submission.id}`, {
    ...
    <div className="min-h-screen bg-slate-50/50 p-4 md:p-6 text-slate-800">
      <div className="max-w-5xl mx-auto space-y-4">
        ...
        <div className="flex items-center justify-between">
            <BackButton href={backLink} label="Back to Roster" className="mb-0" />
            <div className="hidden md:flex items-center gap-3">
                <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Tracking ID</span>
                <span className="text-[10px] font-mono font-bold text-slate-600 bg-slate-200/50 px-2 py-0.5 rounded uppercase tracking-tight">ASG-{submission.id.slice(-6)}</span>
            </div>
        </div>
          ...
          <div className="lg:col-span-2 space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-700">
            ...
            <div className="bg-white rounded-xl p-5 border border-slate-200 shadow-sm space-y-3 relative overflow-hidden group">
                ...
                <div className="relative z-10 space-y-0.5">
                    <p className="text-[8px] font-black text-blue-600 uppercase tracking-widest">Assignment context</p>
                    <h1 className="text-xl font-black text-slate-900 tracking-tight leading-tight">{assignment?.title}</h1>
                    <p className="text-slate-500 text-[11px] font-medium max-w-xl leading-relaxed opacity-80">{assignment?.description}</p>
                </div>
            </div>
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 pb-5 border-b border-slate-100">
                    ...
                    <div className="flex flex-col items-end gap-1 text-right">
                        ...
                        <span className="text-[8px] text-slate-400 font-bold uppercase tracking-widest">
                            {submission.submittedAt ? `Lodged: ${new Date(submission.submittedAt).toLocaleDateString()}` : 'Not Lodged'}
                        </span>
                    </div>
                </div>
                ...
                <div className="space-y-5">
                    <div className="space-y-2">
                        <div className="flex items-center gap-2 px-1">
                            ...
                            <label className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Text Response</label>
                        </div>
