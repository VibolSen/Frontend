...
export default function ExamDetailView({ initialExam, loggedInUser }) {
  ...
  const handleSaveGrade = async () => {
    ...
    try {
      const updatedSubmission = await apiClient.put(`/teacher/exam-submissions/${submissionToGrade.id}`, {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50/30 p-4">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between mb-5">
          <Link href={backLink} prefetch={false} className="flex items-center bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-lg px-4 py-2 font-medium shadow-md hover:shadow-lg transform hover:scale-105 transition-all duration-200">
                  ...
                  {exam.submissions.map((submission) => (
                    <tr key={submission.id}>
                      ...
                      <td className="px-4 py-3 whitespace-nowrap text-sm text-slate-500">
                        <span
                          className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${submission.status === "GRADED"
