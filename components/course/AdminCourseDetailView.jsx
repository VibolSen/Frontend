...
export default function AdminCourseDetailView({ courseId }) {
  ...
  useEffect(() => {
    const fetchCourseDetails = async () => {
      try {
        ...
        // Simulating some institutional stats if not provided by base API
        setStats({
          totalStudents: data.enrollments?.length || 0,
  ...
  if (!course) {
    return (
      <div className="p-12 text-center bg-white rounded-[2.5rem] border border-slate-200 shadow-sm">
        <AlertCircleIcon className="mx-auto h-12 w-12 text-rose-300 mb-4" />
        <h2 className="text-xl font-bold text-slate-900">Academic Entity Not Found</h2>
        <p className="text-slate-500 mt-2 mb-8">The requested course ID could not be located in the institutional database.</p>
        <Link href="/admin/courses" className="inline-flex items-center px-6 py-2.5 bg-blue-600 text-white text-sm font-bold rounded-xl hover:bg-blue-700 transition-all shadow-lg shadow-blue-200">
          <ArrowLeftIcon size={16} className="mr-2" /> Return to Directory
        </Link>
      </div>
    );
  }
    ...
    <div className="space-y-6 max-w-6xl">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-4">
          <Link href="/admin/courses" className="group inline-flex items-center gap-3">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        {[
          ...
          <motion.div
            ...
            className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm flex flex-col justify-between"
          >
          ...
          <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
            <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-lg font-black text-slate-900 tracking-tight">Institutional Configuration</h2>
              </div>
            </div>
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-[2rem] border border-slate-100">
                    <div className="w-12 h-12 bg-white rounded-2xl shadow-sm border border-slate-200 flex items-center justify-center text-blue-600 font-black">
                 ...
                 <div className="max-h-[220px] overflow-y-auto pr-2 space-y-2">
                    {course.groups?.length > 0 ? (
                      course.groups.map(group => (
                        <div key={group.id} className="flex items-center justify-between p-4 bg-white border border-slate-100 rounded-2xl hover:border-blue-200 transition-colors group cursor-default">
                          <div className="flex items-center gap-3">
