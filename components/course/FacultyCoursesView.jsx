...
export default function FacultyCoursesView({ loggedInUser }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [coursesData, departmentsData] = await Promise.all([
        apiClient.get(`/faculty/my-courses?teacherId=${teacherId}`),
  ...
  const handleSave = async (courseData) => {
    ...
    try {
      if (isEditing) {
        await apiClient.put(`/courses?id=${editingCourse.id}`, dataToSend);
      } else {
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsLoading(true);
    try {
      await apiClient.delete(`/courses?id=${itemToDelete.id}`);
